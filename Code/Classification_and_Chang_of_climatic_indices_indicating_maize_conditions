...
# PART v:   Calculating and classifying Dry Spells during vegetation period
...
## NOTE: 
# Dry Spells (DS) Analysis identifies consecutive dry days (Dry day = prec < 0.85 (Barron et al. 2003)) during the vegetation period for each year. 
...
## creation of different data sets we are going to need
dry_spells <- as.data.frame(prec, xy = T)[, 1:2]  # creating data frame with x and y column
n <- c(substr(names(T_mean[[1]]), 1, 4):substr(names(T_mean[[nlyr(T_mean)]]), 1, 4)) # vector with years. used to name new data
ts <- seq(3,ncol(table_start_end_day), 2) # vector we need to specify the column for the start day of vegetation period for each year
te <- seq(4,ncol(table_start_end_day), 2) # vector we need to specify the column for the end day of vegetation period for each year
...
## the loop to calculate and classify the Dry spells 
for (j in 1:(ncol(table_prec)/365)) {
  ...
  dry_spells <- cbind(dry_spells, fe) # store results 
}

names(dry_spells)[3:ncol(dry_spells)] <- n # assign year names to the classification 
...
## save raster stack 
writeRaster(dry_spells_rast, paste0(path,"/Dry_Spell_Classification_", substr(names(T_mean[[1]]), 1, 4), "_", substr(names(T_mean[[nlyr(T_mean)]]), 1, 4), ".grd"), filetype = "RRASTER", overwrite = TRUE)
...
dry_spells_rast <- rast("./02_Daten/Dry_Spell_Classification_2021_2100.grd")
...
## second loop to do the final crop classification, now adding the Dry spell classes
